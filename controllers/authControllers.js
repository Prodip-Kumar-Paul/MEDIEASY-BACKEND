...
export const signupController = async (req, res, next) => {
  try {
    const { hospitalEmail,hospitalPassword } =
      req.body;
    let loginBody ={} ;
    if (hospitalEmail) {
    ...
    const alreadyLoggedIn = await Hospital.findOne({hospitalEmail:hospitalEmail});
    if(alreadyLoggedIn){
...
export const loginHospital = async (req,res,next)=>{
  try{
    ...
    const hospital = await Hospital.findOne({hospitalEmail:hospitalEmail ,verified:true ,isDeleted:false});
    if(!hospital){
      return res.status(406).json({
        status:false,
        message:"Hospital not found",
    ...
    const matchedPassword = await bcryptJs.compare(hospitalPassword,hospital.hospitalPassword);
